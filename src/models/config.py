...
class ModelRegistry:
    ...
    def _get_model_deployment_config(
        self, model_name: str
        ...
        """
        specific_deployment = os.getenv(
            f"{ENV_AZURE_DEPLOYMENT_PREFIX}{model_name.upper().replace('-', '_')}"
        )
    ...
    def _create_overridden_config(
        self, base_config: ModelConfig, deployment_name: str
    ) -> ModelConfig:
        """
    ...
    def _load_environment_overrides(self) -> None:
        ...
        for name in AZURE_MODELS:
            ...
            if deployment_name:
                ...
                overridden_config = self._create_overridden_config(
                    base_config, deployment_name
                )
    ...
    def _ensure_model_provider(self, model: ModelConfig) -> ModelConfig:
        ...
        if isinstance(model.provider, str):
            try:
                ...
                raise ValueError(
                    ERROR_INVALID_PROVIDER.format(
                        provider=model.provider, valid_providers=list(ModelProvider)
                    )
                )
        elif not isinstance(model.provider, ModelProvider):
            raise ValueError(
                ERROR_PROVIDER_TYPE.format(provider_type=type(model.provider))
            )
        return model
    ...
    def get_model(
        ...
        if "/" in name:
            ...
            try:
                ...
                raise ValueError(
                    ERROR_MODEL_NOT_FOUND.format(name=name, available=available)
                )
        ...
        if len(matches) > 1:
            ...
            raise ValueError(
                ERROR_AMBIGUOUS_MODEL.format(
                    name=name, conflict_details=conflict_details
                )
            )
        return matches[0]
    ...
    def validate_model_config(self, config: ModelConfig):
        ...
        is_built_in = (
            config in AZURE_MODELS.values() or config in OPENAI_MODELS.values()
        )
        ...
        if config.provider == ModelProvider.AZURE:
            specific_deployment, global_override = self._get_model_deployment_config(
                config.name
            )
            ...
            if (
                specific_deployment
                and global_override
                and specific_deployment != global_override
            ):
                if is_built_in:
                    logging.warning(
    ...
    def list_models(
        ...
        Performance Note:
            Uses generator expressions for memory-efficient filtering
        """
        ...
        if provider:
            if not isinstance(provider, ModelProvider):
                raise ValueError(
                    ERROR_INVALID_PROVIDER.format(
                        provider=provider, valid_providers=list(ModelProvider)
                    )
                )
        ...
        if capability:
            ...
            elif capability in self.VALID_CAPABILITIES:
                models = (
